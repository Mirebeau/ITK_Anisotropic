 ...
 *
 *=========================================================================*/
#ifndef __itkFastMarchingImageFilter_Modified_hxx
#define __itkFastMarchingImageFilter_Modified_hxx
...
#include <algorithm>
#include <limits> // Added by Mirebeau January 7th 2013

namespace itk
{
template< class TLevelSet, class TSpeedImage >
FastMarchingImageFilter_Modified< TLevelSet, TSpeedImage >
...
namespace {
// Mirebeau Septembre 13th 2013.
// Introduced to allow subclassing FastMarchingImageFilter, to handle anisotropic propagation speeds.
    template<typename TSpeedImage>
    struct SpeedToDouble {
        typedef typename TSpeedImage::PixelType SpeedType;
        
        template <bool,class Dummy> struct SpeedToDouble_;
        
        template <class Dummy> struct SpeedToDouble_<true,Dummy>{
            double operator ()(SpeedType s){return static_cast<double>(s);}
        };
        
        template <class Dummy> struct SpeedToDouble_<false,Dummy>{
            double operator ()(SpeedType s){return 1;}
        };
        
        double operator()(SpeedType s){return SpeedToDouble_<std::numeric_limits<SpeedType>::is_specialized, void>()(s); }
    };    
  ...
  LevelSetImageType *output)
{
  ...
  if ( speedImage )
    {        
        cc = SpeedToDouble<TSpeedImage>()( speedImage->GetPixel(index)  ) / m_NormalizationFactor;
