...
// The Hopf-Lax update operator unfortunately must be specialized.
    ...
    ExtendedNorm< TNorm >::Hopf_Lax(typename DistanceImageType::Pointer d, const IndexType &x,
                                             AcceptedImagePointerType b, unsigned int y_i,
                                             const Stencil & stencil,
                                             IndicesType &gradient_indices, WeightsType &gradient_weights) const
    {
        return HL::GoodSubclass::Hopf_Lax(this, d, x, b, y_i, stencil, gradient_indices, gradient_weights);
    }
    
#pragma remark("Replace this ugly template mess with some 
    
    template<typename TNorm>
    struct ExtendedNorm<TNorm>::HL{
        
        template<class TNorm2, class Dummy> struct HL2;
        
        template<class Dummy> struct HL2<Finsler2DNorm,Dummy>{
            template<typename AcceptedImagePointerType>
            static ValueType Hopf_Lax(const Self *pNorm,
                                      typename DistanceImageType::Pointer d, const IndexType &x,
                                      AcceptedImagePointerType b, unsigned int y_i,
                                      const Stencil & stencil,
                                      IndicesType &gradient_indices, WeightsType &gradient_weights) 
            {
                ...
                if(y_i<primarySize){
                    ...
                    const OffsetType Oy = PrimaryShortOffset_to_Offset(BSOy);
                    ...
                    for(int e=primarySize; e<stencil.size(); ++e){
                        ...
                        OffsetType Op = ShortOffset_to_Offset(BSOp);
                    ...
                    for(int e=-1; e<=1; e+=2){
                        ...
                        OffsetType Oz = PrimaryShortOffset_to_Offset(BSOz);
                    ...
                    OffsetType Op = ShortOffset_to_Offset(SOp);
                    const IndexType p = x+Op;
                    ...
                    for(int y_i=0; y_i<primarySize; ++y_i){
                        ...
                        OffsetType Oy = PrimaryShortOffset_to_Offset(BSOy);
                        IndexType y = x+Oy;
        ...
        template<class Dummy> struct HL2<Riemannian2DNorm,Dummy>{
            
            // If this is assembled, then PrimaryNorm equals Riemannian2DNorm.
            template<typename AcceptedImagePointerType>
            static ValueType Hopf_Lax(const Self *pNorm,
                                      typename DistanceImageType::Pointer d, const IndexType &x,
                                      AcceptedImagePointerType b, unsigned int y_i,
                                      ...
                                      IndicesType &gradient_indices, WeightsType &gradient_weights)
            {
                ...
                if(y_i<primarySize){
                    ...
                    const OffsetType Op = ShortOffset_to_Offset(SOp);
                    const IndexType p = x+Op;
                    const ValueType Dp = d->GetPixel(p);
                    const ValueType N2p = H;
                    ...
                    for(int y_i=0; y_i<primarySize; ++y_i){
                        ...
                        const int z_i= (y_i+1)%primarySize;
                        const PrimaryShortOffsetType BSOz = primaryStencil[z_i];
                        const OffsetType Oz = PrimaryShortOffset_to_Offset(BSOz);
                        const IndexType z = x+Oz;
        ...
        }; // End of Riemannian2DNorm specialization
        
        
        
        template<class Dummy> struct HL2<Riemannian3DNorm,Dummy>{
            template<typename AcceptedImagePointerType>
            static ValueType Hopf_Lax(const Self *pNorm,
                                      typename DistanceImageType::Pointer d, const IndexType &x,
                                      AcceptedImagePointerType b, unsigned int y_i,
                                      ...
                                      IndicesType &gradient_indices, WeightsType &gradient_weights)
            {
                ...
                if(y_i<primarySize){
                    ...
                    for(int i=0; i<nN; ++i){
                        ...
                        const IndexType z=x+PrimaryShortOffset_to_Offset(BSOz);
        ...
        }; // Subclass Riemannian3DNorm specialization
        
        typedef HL2<PrimaryNorm,void> GoodSubclass;
    };
